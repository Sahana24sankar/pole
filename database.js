    ...
    try {
      ...
      return {
        ...
        data: { connected: false, status: 'unhealthy' }
          ...
          async insertAd(poleType, imagePath, createdBy = 'web-user', expiryDate = null) {
            try {
              console.log(`📡 Inserting ad: pole=${poleType}, image=${imagePath}, user=${createdBy}, expiry=${expiryDate}`);
              
              const result = await pool.query(`
                INSERT INTO pole_images (poletype, image, isactive, createddate, createdby, expirydate) 
                VALUES ($1, $2, $3, NOW(), $4, $5) 
                RETURNING id, poletype, image, isactive, createddate, createdby, expirydate
              `, [poleType, imagePath, true, createdBy, expiryDate]);
